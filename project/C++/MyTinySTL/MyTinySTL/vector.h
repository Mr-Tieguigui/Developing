...
public:
  ...
  typedef const value_type*                        const_iterator;               // const迭代器(const指针)
...
public:
  // 构造、复制、移动、析构函数
  // 无参构造函数
  vector() noexcept   // noexcept 指定某个函数不抛出异常
  { try_init(); }

  // 有参构造函数
  explicit vector(size_type n)
  { fill_init(n, value_type()); }            // 调用基本类型，如int，的构造函数，所以值为0
  ...
  input iterator是五种迭代器的一种，只允许向前移动读数据
  */
  ...
  // 拷贝构造函数
  /*
  类的成员函数可以访问这个类的私有数据成员，要理解这个对类的访问限制，而不是针对对象
  vector作为成员函数的参数时，不用加<T>
  但是成员函数在类外定义时返回值需要指定<T>，参数仍然不需要
  */
  ...
  // 移动构造函数因为肯定会改变传入对象，所以和拷贝构造函数不同，参数不能是const
  vector(vector&& rhs) noexcept  // 仅仅是指针的赋值，并没有申请内存，并且把右值的指针置空
    :begin_(rhs.begin_),
    end_(rhs.end_),
    cap_(rhs.cap_)
  {
...
vector<T>& vector<T>::operator=(const vector& rhs)
{
  if (this != &rhs)            // 不处理自己给自己赋值
  {
    ...
    if (len > capacity())      // 当前vector容量不够，就需要扩容
    { 
    ...
    else if (size() >= len)    // 当前vector内容比rhs长，需要将当前vector多余的内容销毁
    {
      ...
      data_allocator::destroy(i, end_);   // 删除范围内的元素，但并不会释放空间
      end_ = begin_ + len;
    }
    else
    { 
...
void vector<T>::try_init() noexcept
{
  try
  {
    begin_ = data_allocator::allocate(16); // 默认分配16个元素空间，begin_指向已使用空间的开头，静态函数的调用方式
...
{
  data_allocator::destroy(first, last);      // 删除范围内的元素，但并不会释放空间
  data_allocator::deallocate(first, n);      // 释放空间
}

// get_new_cap 函数: 没有多余容量时需要用此函数扩容
template <class T>
...
copy_assign(IIter first, IIter last, input_iterator_tag)
{
  ...
  if (first == last)    // 如果size比迭代器范围大，说明后面还y有内容，应该删掉
  {
    erase(cur, end_);
  }
  else
  {
    insert(end_, first, last);   // 继续复制迭代器范围内的元素
  }
}
